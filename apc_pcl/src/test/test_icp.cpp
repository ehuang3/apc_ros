...
int main (int argc, char** argv)
{
    ...
    cloud_from_ply(filename, *target_cloud);

    Eigen::Vector3f origin, normal;
    origin << 0, 0, 0;
    normal << 1, 1, 0;

    discard_halfspace(normal, origin, *target_cloud, *input_cloud);
    affine_cloud(Eigen::Vector3f::UnitX(), 0.2, Eigen::Vector3f(-0.1, -0.3, 0.0), *input_cloud, *input_cloud);
    ...
    *input_copy = *input_cloud;
    ...
    while(!viewer.wasStopped()) {
        ...
        if ((steps % 30) == 0) {
            ...
            icp_result result;
            if (go_on) {
                result = apply_icp(input_cloud, target_cloud);
            } else {
                continue;
            }
